...
const MessageList = () => {
  ...
  return (
    <div className="flex  min-h-screen p-6 bg-gray-100 w-full">
      {selectedMessage ? (
        <MessageDetail message={selectedMessage} setSelectedMessage={setSelectedMessage} />
      ) : (
          ...
          {messages.map((message) => (
            ...
            >
              <div className='flex flex-row justify-between '>
                <div className="font-semibold text-gray-900">{message.subject}</div>
                <div className="text-gray-500">{message.from}</div>
              </div>
              <div className="text-sm text-gray-400">{message.body.slice(0, 50)}...</div>
            </div>
          ))}
        </div>
      )}
